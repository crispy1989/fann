...
FANN_EXTERNAL float FANN_API fann_test_data(struct fann *ann, struct fann_train_data *data)
{
	...
	fann_begin_log("fann_test_log_%d", tdctr);
	fann_log("BEGINNING fann_test_data()\n");

	sprintf(fn, "fann_test_data_%d", tdctr);
	printf("fann_test_data() called\n");
	fann_save_train(data, fn);
	sprintf(fn, "fann_test_ann_%d", tdctr);
	fann_save(ann, fn);

	if(fann_check_input_output_sizes(ann, data) == -1)
		return 0;

	fann_log("fann_reset_MSE()\n");
	fann_reset_MSE(ann);

	fann_log("MSE is now %f\n", fann_get_MSE(ann));

	for(i = 0; i != data->num_data; i++)
	{
		fann_log("=== fann_test() %d/%d ===\n", i + 1, data->num_data);
		fann_test(ann, data->input[i], data->output[i]);
		fann_log("=== fann_test() finished, MSE is %f\n", fann_get_MSE(ann));
	}

	fann_log("!!! FINAL MSE: %f\n", fann_get_MSE(ann));

	printf("mse is %f\n", fann_get_MSE(ann));

	sprintf(fn, "fann_test_data_after_%d", tdctr);
	fann_save_train(data, fn);
	sprintf(fn, "fann_test_ann_after_%d", tdctr);
	fann_save(ann, fn);
